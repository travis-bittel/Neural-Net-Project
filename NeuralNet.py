...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """
        """YOUR CODE"""
        acts = copy.deepcopy(inActs)
        acts.insert(0, 1) # Add 1 to start of inActs for bias input
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for weight in self.weights:
            newWeight = weight + alpha * acts[i] * delta
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        """YOUR CODE"""
        outerList = []
        outerList.append(inActs)

        for layer in self.layers:
            innerList = []
            for perceptron in layer:
                innerList.append(perceptron.sigmoidActivation(inActs))
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    sum = 0
                    i = 0
                    for perceptron in nextLayer:
                        sum += perceptron.weights[neuronNum + 1] * deltas[0][i]
                        i += 1
                    delta = gPrime * sum
